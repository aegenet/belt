 ...
 */
export function getClosestNumber(value: number, collection: number[]): number {
  let aDiff: number;
  let bDiff: number;
 ...
 * Get closest value
 */
export function getClosestValue<T = number>(value: T, collection: T[], reducer: (value: T, a: T, b: T, collection: T[]) => T): T {
