...
export async function warOfSort(duration: number, arraySize: number): Promise<RaceResult[]> {
  ...
  const mergeSort = (recipes: Array<{ name: string }>) => {
    if (recipes.length > 1) {
      const middleLength = Math.floor(recipes.length / 2);
      const leftSide = recipes.slice(0, middleLength);
      const rightSide = recipes.slice(middleLength);
      mergeSort(leftSide);
      mergeSort(rightSide);
      let leftIndex = 0;
      let rightIndex = 0;
      ...
      while (leftIndex < leftSide.length && rightIndex < rightSide.length) {
        if (leftSide[leftIndex].name < rightSide[rightIndex].name) {
          recipes[globalIndex] = leftSide[leftIndex];
          leftIndex++;
        } else {
          recipes[globalIndex] = rightSide[rightIndex];
          rightIndex++;
        }
        globalIndex++;
      }
